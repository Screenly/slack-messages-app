...
| `override_locale`   | string | No       | Override locale with a [BCP 47](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl#locales_argument) tag (e.g. `en`, `fr`, `de`). Default: `en` |
| `override_timezone` | string | No       | Override timezone with an [IANA time zone](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) (e.g. `Europe/London`). Defaults to the device timezone if blank          |
